            ...
            if(currentSpawnCount == 0)
            {
                ...
                if (currentWaveIndex % 10 == 0)
                {
                    waveSpawnPosCount = waveSpawnPosCount + 1 
                        > spawnPositions.Count ? waveSpawnPosCount : waveSpawnPosCount + 1;
                }
                ...
                for(int i=0; i<waveSpawnPosCount; ++i)
                {
                    ...
                    for(int j=0; j<waveSpawnCount; j++)
                    {
                        ...
                        GameObject enemy = Instantiate(enemyPrefebs[prefabIdx], 
                            spawnPositions[posIdx].position, Quaternion.identity);
                        enemy.GetComponent<HealthSystem>().OnDeath += OnEnemyDeath;
                        enemy.GetComponent<CharacterStatsHandler>().AddStatModifer(defaultStats);
                        enemy.GetComponent<CharacterStatsHandler>().AddStatModifer(rangedStats);
                        currentSpawnCount++;
